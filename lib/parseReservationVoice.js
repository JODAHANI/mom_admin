...
// 지원: 날짜(년월일/상대), 시간(시분/오전오후), 연락처, 인원(어른/어린이), 메뉴+수량, 이름, 특이사항.
...
function parsePhone(text) {
  ...
  return { phone: formatted, match: m[0] };
}
...
export function parseReservationVoice(transcript, { baseDate = new Date() } = {}) {
  ...
  const notesLabel = parseNotesLabel(residual);
  if (notesLabel) {
    fields.notes = notesLabel.notes;
    filled.notes = true;
    residual = residual.replace(notesLabel.match, ' ');
  }
  ...
  if (!fields.notes && residual.length > 1) {
    fields.notes = residual;
    filled.notes = true;
  }
